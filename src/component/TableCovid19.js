...
import Covid19 from '../hook/useCovid19';
...
function createData(Country, NewConfirmed, NewDeaths, NewRecovered, TotalConfirmed, TotalDeaths, TotalRecovered) {
    return { Country, NewConfirmed, NewDeaths, NewRecovered, TotalConfirmed, TotalDeaths, TotalRecovered };
}
export default function SimpleTable() {
    const classes = useStyles();
    let dataCovid19 = Covid19();
    let dataTable = dataCovid19[0].Countries;
    let rows = [];
    if (dataTable != undefined) {
        dataTable.forEach(el => {
            rows.push(createData(el.Country, el.NewConfirmed, el.NewDeaths, el.NewRecovered, el.TotalConfirmed, el.TotalDeaths, el.TotalRecovered))
        });
    }
